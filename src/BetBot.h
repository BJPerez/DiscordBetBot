...
private:
	...
	[[nodiscard]] dpp::slashcommand CreateClearDataCommand() const { return { "clear_data", "Clear all the saved data (matches and bets).", m_Cluster.me.id }; }
	...
	void ExecuteShowNextIncomingMatches(const dpp::button_click_t& event);
	void ExecuteClearData(const dpp::slashcommand_t& event);
